   ...
   const evm_uint256be key) const
{
   ...
   if (secp256k1_ecdsa_sign_recoverable(ctx, &signature, hash.bytes, key.bytes, nullptr, nullptr)) {
      unsigned char signatureTemp[64];
      int version;
      secp256k1_ecdsa_recoverable_signature_serialize_compact(ctx, signatureTemp, &version, &signature);

      //TODO: need to compare to a large number statically defined in cpp-ethereum; is it just an overflow check?
      uint8_t uv = (uint8_t)version;
      serializedSignature.push_back(uv);
      std::copy(signatureTemp, signatureTemp+64, std::back_inserter(serializedSignature));
   }
 ...
 */
evm_address com::vmware::concord::EthSign::ecrecover(const evm_uint256be hash,
                                                    const uint8_t version,
                                                    const evm_uint256be r,
                                                    const evm_uint256be s) const
{
   // parse_compact supports 0-3, but Ethereum is documented as only using 0 and 1
   if (version > 1) {
      return zero_address;
   }
