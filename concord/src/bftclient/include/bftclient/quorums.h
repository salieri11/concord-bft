...
// A quorum of 2F + C + 1 matching replies from `destination` must be received
// for the `send` call to complete.
struct LinearizableQuorum {
  std::set<ReplicaId> destinations;
};

// A quorum of F + 1 matching replies from `destination` must be received for
// the `send` call to complete.
struct ByzantineSafeQuorum {
  std::set<ReplicaId> destinations;
};

// A matching reply from every replica in `destination` must be received for the
// `send` call to complete.
struct All {
  std::set<ReplicaId> destinations;
};

// A matching reply from `wait_for` number of replicas from `destination` must
// be received for the `send` call to complete.
struct MofN {
  size_t wait_for;
  std::set<ReplicaId> destinations;
};
...
typedef std::variant<LinearizableQuorum, ByzantineSafeQuorum, All, MofN>
    ReadQuorum;
...
class QuorumConverter {
 public:
  QuorumConverter(const std::set<ReplicaId>& all_replicas, uint16_t f_val,
                  uint16_t c_val)
      : all_replicas_(all_replicas), f_val_(f_val), c_val_(c_val) {}
