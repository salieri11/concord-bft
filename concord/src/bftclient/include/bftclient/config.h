...
// This config is based on the parameters to the
// DynamicUpperLimitWithSimpleFilter. The defaults are set to the original
// defaults in SimpleClientImp.
struct RetryTimeoutConfig {
  ...
  // The number of standard deviations within the average that we expect reply
  // times to fall into.
  uint16_t number_of_standard_deviations_to_tolerate = 2;

  // The number of replies we sample before we attempt to recalculate the
  // rolling average and variance.
  uint16_t samples_per_evaluation = 32;

  // The number of samples before we reset the rolling average and variance to
  // its empty state A negative number will turn off resets.
  int16_t samples_until_reset = 1000;

  // The scaling factor at which the timeout can be increased. A factor of 2
  // means that the upper limit can be doubled on each evaluation period.
  double max_increasing_factor = 2;

  // The scaling factor at which the timeout can be decreased. A factor of 2
  // means that the upper limit can be halved on each evaluation period.
...
struct ReadConfig {
  RequestConfig request;
  ReadQuorum quorum = LinearizableQuorum();
};
