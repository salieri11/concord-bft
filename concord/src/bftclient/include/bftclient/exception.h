...
class BadQuorumConfigException : public BftClientException {
 public:
  BadQuorumConfigException(const std::string& what)
      : BftClientException(what) {}
};

class InvalidDestinationException : public BftClientException {
 public:
  InvalidDestinationException(ReplicaId replica_id)
      : BftClientException("Replica: " + std::to_string(replica_id.val) +
                           " is not part of the cluster.") {}

  InvalidDestinationException()
      : BftClientException("MofN quorums must have destinations") {}
};
...
class TimeoutException : public BftClientException {
 public:
  TimeoutException(uint64_t seq_num, std::string cid)
      : BftClientException(
            "Timeout for request sequence number: " + std::to_string(seq_num) +
