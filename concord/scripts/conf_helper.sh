...
if  [[ $1 = "-s" ]]; then
	CONF_FILE="$CONF_PATH/secrets.config"
	echo "looking for $CONF_FILE"
	if [ ! -f $CONF_FILE ]; then
    		echo "can't find $CONF_FILE"
		exit
	...
	sshpass -p $PASS ssh -o StrictHostKeyChecking=no root@$var bash -s <<'EOF'
	DATE=$(date +%s)
	cp /config/concord/config-local/secrets.config /config/concord/config-local/secrets.config_$DATE 2>/dev/null
  	cp /tmp/secrets.config /config/concord/config-local
	chmod 666 /config/concord/config-local/secrets.config
  	exit
EOF
	done
	exit
...
if  [[ $1 = "-d" ]]; then
	CONF_FILE="$CONF_PATH/deployment.config"
	echo "looking for $CONF_FILE"
	if [ ! -f $CONF_FILE ]; then
    		echo "can't find $CONF_FILE"
		exit
	...
	sshpass -p $PASS ssh -o StrictHostKeyChecking=no root@$var bash -s <<'EOF'
	DATE=$(date +%s)
	cp /config/concord/config-local/deployment.config /config/concord/config-local/deployment.config_$DATE 2>/dev/null
  	cp /tmp/deployment.config /config/concord/config-local
	chmod 666 /config/concord/config-local/deployment.config
  	exit
EOF
	done
	exit
