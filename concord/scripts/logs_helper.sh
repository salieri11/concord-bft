...
if  [[ $1 = "-h" ]] || [ -z $1 ]; then
	...
	echo "example: PRP_PATH=. ./logs_helper.sh -p moshe -c 10.70.30.34"
	echo ""
	echo "[-m] distruibutes log4cplus.properties from the given path to the given replicas"
	echo "example: PRP_PATH=./log4prop ./logs_helper.sh -p moshe -m 10.70.30.34 10.73.232.21"
	echo ""
	echo "[-f] fetches application logs from replicas and prticipant (if supplied)"
	echo "example: PARTICIPANT=10.40.205.63 ./logs_helper.sh -p moshe -f 10.40.205.56 10.40.205.57"
	echo ""
