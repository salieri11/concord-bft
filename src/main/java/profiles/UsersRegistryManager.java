...
public class UsersRegistryManager {
   ...
   private List<User> getUsersInternal(String consortiumID,
      ...
      if (org.isPresent() && con.isPresent()) {
         return userRepository.findUsersByConsortiumAndOrganization(
                 con.get(), org.get());
      } else {
   ...
   public String createUser(String name, String email, String role,
      ...
      if (o.isPresent() && c.isPresent() && Roles.contains(role)) {
         ...
         if (fName.isPresent() && lName.isPresent()) {
            u = new User(name, email, fName.get(),
                    lName.get(), Roles.fromString(role),
                    o.get(), c.get(), password);
         } else {
            u = new User(name, email, Roles.fromString(role),
                    o.get(), c.get(), password);
         }
   ...
   public void updateUser(UserPatchRequest upr) throws
           UserModificationException {
      Optional<User> oUser =
    ...
    public boolean loginUser(String userID, String password) throws
            UserModificationException {
   ...
   public Long createConsortiumIfNotExist() {
      ...
      if (cList.isEmpty()) {
         Consortium c = new Consortium("TEST_CON",
                 "ATHENA");
