...
class RLPParser {
   Logger logger = Logger.getLogger(RLPParser.class);
   ...
   private ByteString longRun(int lengthLength) throws RLPEmptyException {
      ...
      /*
       * ByteString only supports 32-bit integer length. If the RLP specifies
       * something larger than that, it's probably lying, or Helen is probably
       * already crashing if it got this far. Skip the length bytes above the
       * 4-byte limit, and then go on to read the length they say to read. If we
       * continue after this, the request will be rejected when EthSendTxHandler
