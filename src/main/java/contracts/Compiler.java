...
public class Compiler {
    ...
    * file inside that directory. The output files generated by the compiler
    * will also be present in same temporary directory
    * 
    * @param contents
   ...
   /**
    * Checks if the given Path represents a solidity compiler generated bytecode
    * file. Currently the extension of solidity compiler generated bytecode file
    * is `.bin`.
    * 
    * @param file
    *           Path of the bytecode file
   ...
   /**
    * Checks if the given Path represents a solidity compiler generated metadata
    * file. Currently solidity compiler appends `_meta` to the name of the
    * contract and then generates a metadata file with name as
    * `<contract_name>_meta.json`. Here we simply check if given path ends in
    * `_meta.json`.
    * 
    * @param file
   ...
   /**
    * Extracts the name of the contract from given Path of the bytecode file (
    * <ContractName>.bin) or from the given Path of the metadata
    * (<ContractName>_meta.json) file.
    * 
    * @param file
    *           Path of the bytecode or metadata file
   ...
   /**
    * Reads all contents of a file into a string. Note: The contents are first
    * read into a byte array and then converted into a string by using systems
    * default charset.
    * 
    * @param file
    *           Path of the file
   ...
   /**
    * Reads all the byteCode files in given directory and returns a Map with key
    * as the name of the contract and value as the contents of its bytecode (one
    * map entry per bytecode file).
    * 
    * @param dir
   ...
   /**
    * Reads all the metadata files in given directory and returns a Map with key
    * as the name of the contract and value as the contents of its metadata (one
    * map entry per metadata file)
    * 
    * @param dir
    ...
    * compilation. This method invokes the pre-installed `solc` command. Hence,
    * we first have to write the give solidity contract code into a file which
    * we then pass to `solc` as a command line argument.
    * 
    * @param solidityCode
    *           The string containing solidity source code
