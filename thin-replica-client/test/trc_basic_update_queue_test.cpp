...
unique_ptr<Update> MakeUniqueUpdate(
    uint64_t block_id, const vector<pair<string, string>> kv_pairs) {
...
TEST(trc_basic_update_queue_test, test_constructor) {
  ...
  EXPECT_NO_THROW(queue.reset(new BasicUpdateQueue()))
      << "BasicUpdateQueue's default constructor failed.";
}

TEST(trc_basic_update_queue_test, test_destructor) {
  auto queue = make_unique<BasicUpdateQueue>();
  EXPECT_NO_THROW(queue.reset())
      << "BasicUpdateQueue's destructor failed to destruct an empty queue.";

  queue.reset(new BasicUpdateQueue());
  queue->Push(MakeUniqueUpdate(0, vector<pair<string, string>>{{"a", "a"}}));
  queue->Push(
      MakeUniqueUpdate(1, vector<pair<string, string>>{{"key", "value"}}));
  EXPECT_NO_THROW(queue.reset())
      << "BasicUpdateQueue's destructor failed to destruct a non-empty queue.";
}

TEST(trc_basic_update_queue_test, test_release_consumers) {
  auto queue = make_unique<BasicUpdateQueue>();
  EXPECT_NO_THROW(queue->ReleaseConsumers())
      << "BasicUpdateQueue::ReleaseConsumers fails when called on a queue with "
...
TEST(trc_basic_update_queue_test, test_clear) {
  ...
  EXPECT_NO_THROW(queue.Clear())
      << "BasicUpdateQueue::Clear fails when called on an already-empty queue.";
  queue.Push(MakeUniqueUpdate(0, vector<pair<string, string>>{{"a", "a"}}));
  queue.Push(MakeUniqueUpdate(1, vector<pair<string, string>>{{"b", "b"}}));
  EXPECT_NO_THROW(queue.Clear())
      << "BasicUpdateQueue::Clear fails when called with a non-empty queue.";
  EXPECT_EQ(queue.Size(), 0)
      << "A BasicUpdateQueue appears to have non-0 size after a call to Clear.";
  EXPECT_FALSE((bool)(queue.TryPop()))
      << "A BasicUpdateQueue appears to pop a non-null element after a call to "
         "Clear.";
}
...
TEST(trc_basic_update_queue_test, test_push) {
  ...
  for (uint64_t i = 1; i <= kNumUpdatesToTest; ++i) {
    EXPECT_NO_THROW(queue.Push(
        MakeUniqueUpdate(i, {{"key" + to_string(i), "value" + to_string(i)}})))
        << "Push fails when pushing to a queue with " << to_string(i)
        << " existing updates.";
  }
}
...
TEST(trc_basic_update_queue_test, test_pop) {
  ...
  thread consumer([&]() {
    EXPECT_NO_THROW(update = queue.Pop())
        << "BasicUpdateQueue::Pop call initiated on an empty queue failed with "
           "an exception.";
    ASSERT_TRUE((bool)update)
        << "BasicUpdateQueue::Pop call initiated on an empty queue returned a "
           ...
           "calls.";
    EXPECT_EQ(update->block_id, 0)
        << "BasicUpdateQueue::Pop call returned an update with a Block ID "
           "not "
           "matching the only update the Pop call should have had access to.";
    EXPECT_EQ(update->kv_pairs.size(), 0)
        << "BasicUpdateQueue::Pop call returned an update with a set of KV "
  ...
  consumer.join();
  queue.Push(
      MakeUniqueUpdate(1, vector<pair<string, string>>{{"key1", "value1"}}));
  queue.Push(
      MakeUniqueUpdate(2, vector<pair<string, string>>{{"key2", "value2"}}));
  EXPECT_NO_THROW(update = queue.Pop())
      << "BasicUpdateQueue::Pop call initiated on a queue with 2 updates "
         "failed with an exception.";
  EXPECT_TRUE((bool)update)
      << "BasicUpdateQueue::Pop call initiated on a queue with 2 updates "
         "returned a null pointer in the absence of any ReleaseConsumers call.";
  EXPECT_NO_THROW(update = queue.Pop())
      << "BasicUpdateQueue::Pop call initiated on a queue with 1 update failed "
         "with an exception.";
  EXPECT_TRUE((bool)update)
      << "BasicUpdateQueue::Pop call initiated on a queue with 1 update "
         "returned a null pointer in the absence of any ReleaseConsumers call.";

  EXPECT_EQ(queue.Size(), 0)
      << "A BasicUpdateQueue was unexpectedly found to have a non-zero size "
         "after a matching number of pushes and pops.";
  queue.Push(
      MakeUniqueUpdate(3, vector<pair<string, string>>{{"key3", "value3"}}));
  queue.Push(
      MakeUniqueUpdate(4, vector<pair<string, string>>{{"key4", "value4"}}));
  EXPECT_NO_THROW(update = queue.TryPop())
      << "BasicUpdateQueue::TryPop call initiated on a queue with 2 updates "
         "failed with an exception.";
  EXPECT_TRUE((bool)update)
      << "BasicUpdateQueue::TryPop call initiated on a queue with 2 updates "
         ...
         "calls.";
  EXPECT_NO_THROW(update = queue.TryPop())
      << "BasicUpdateQueue::TryPop call initiated on a queue with 1 update "
         "failed with an exception.";
  ...
  EXPECT_EQ(queue.Size(), 0)
      << "A BasicUpdateQueue was unexpectedly found to have a non-zero size "
         "after a matching number of pushes and pops.";
  EXPECT_NO_THROW(update = queue.TryPop())
      << "BasicUpdateQueue::TryPop call on an empty queue failed with an "
         "exception.";
  EXPECT_FALSE((bool)update)
      << "BasicUpdateQueue::TryPop call on an empty queue in the absence of "
         "any concurrent or subsequent Push calls returned a non-null update.";
  consumer = thread([&]() {
    update = queue.Pop();
    EXPECT_FALSE((bool)update)
        << "BasicUpdateQueue::Pop call initiated on an empty queue followed by "
...
TEST(test_basic_update_queue_test, test_size) {
  BasicUpdateQueue queue;
  EXPECT_EQ(queue.Size(), 0)
      << "BasicUpdateQueue::Size gives the wrong size for an empty queue.";
  for (int i = 0; i < kNumUpdatesToTest; ++i) {
    queue.Push(MakeUniqueUpdate(i, vector<pair<string, string>>{}));
    EXPECT_EQ(queue.Size(), (i + 1))
        << "BasicUpdateQueue::Size gives the wrong size for a queue to which "
...
TEST(trc_basic_update_queue_test, test_ordering) {
  ...
  thread consumer([&]() {
    for (uint64_t i = 0; i < kNumUpdatesToTest; ++i) {
      ...
      if (i % 2 == 0) {
        EXPECT_NO_THROW(update = queue.Pop())
            << "BasicUpdateQueue::Pop call failed with an exception.";
        ASSERT_TRUE((bool)update)
            << "BasicUpdateQueue::Pop returned a null pointer in the absence "
               "of a concurrent or subsequent call to ReleaseConsumers.";
      } else {
        while (!update) {
          EXPECT_NO_THROW(update = queue.TryPop())
              << "BasicUpdateQueue::TryPop call failed with an exception.";
        }
      }
      EXPECT_EQ(update->block_id, i)
...
void TestNoUpdateDuplicationOrLossProducer(BasicUpdateQueue& queue,
                                           uint64_t producer_index) {
  for (uint64_t i = producer_index; i < kNumUpdatesToTest;
       i += kRacingThreadsToTest) {
    EXPECT_NO_THROW(queue.Push(
        MakeUniqueUpdate(i, vector<pair<string, string>>{
...
void TestNoUpdateDuplicationOrLossConsumer(BasicUpdateQueue& queue,
  ...
  for (uint64_t i = consumer_index; i < kNumUpdatesToTest;
       i += kRacingThreadsToTest) {
    unique_ptr<Update> update(nullptr);
    if (((i / kRacingThreadsToTest) % 2) == 0) {
      EXPECT_NO_THROW(update = queue.Pop())
          << "BasicUpdateQueue::Pop call failed with an exception.";
      ASSERT_TRUE((bool)update)
          << "BasicUpdateQueue::Pop returned a null pointer in the absence of "
    ...
    EXPECT_EQ(update->kv_pairs.size(), 2)
        << "A BasicUpdateQueue popping call returned an update with an "
...
TEST(trc_basic_update_queue_test, test_no_update_duplication_or_loss) {
  ...
  for (uint64_t i = 0; i < kRacingThreadsToTest; ++i) {
    threads.emplace_back(TestNoUpdateDuplicationOrLossConsumer, ref(queue),
                         ref(observed_updates_by_thread[i]), i);
  }
    ...
    for (const auto& update_observed : observed_by_thread) {
      ...
      if (update_observed < updates_observed.size()) {
        EXPECT_FALSE(updates_observed[update_observed])
            << "A BasicUpdateQueue appears to have duplicated an update.";
        updates_observed[update_observed] = true;
      }
    }
  }
  for (const auto& update_observed : updates_observed) {
    EXPECT_TRUE(update_observed)
        << "A BasicUpdateQueue appears to have lost an update.";
  }
}
