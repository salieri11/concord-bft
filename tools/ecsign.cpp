...
std::vector<uint8_t> next_part(athena::RLPParser &parser, const char *label)
{
   if (parser.at_end()) {
...
int main(int argc, char **argv) {
   ...
   string tx_s(argv[1]);
   vector<uint8_t> tx = athena::dehex(tx_s);

   string key_s(argv[2]);
   vector<uint8_t> key_v = athena::dehex(key_s);
   if (key_v.size() != sizeof(evm_uint256be)) {
      cerr << "Key hex not long enough (is " << key_v.size()
           << " bytes, must be " << sizeof(evm_uint256be) << " bytes)" << endl;
      return -1;
   }
   evm_uint256be key;
   ...
   athena::RLPParser tx_envelope_p = athena::RLPParser(tx);
   ...
   athena::RLPParser tx_parts_p = athena::RLPParser(tx_envelope);
   ...
   evm_uint256be txhash = athena::EthHash::keccak_hash(tx);
   athena::EthSign verifier;
   ...
   if (signature.size() != 1+2*sizeof(evm_uint256be)) {
      cerr << "Signature is not expected length (was " << signature.size()
           << " bytes, expected " << (1+2*sizeof(evm_uint256be)) << ")" << endl;
      return -1;
   }
   ...
   std::copy(signature.begin()+1+sizeof(evm_uint256be),
             signature.end(),
             std::back_inserter(s));
