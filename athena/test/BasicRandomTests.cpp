...
{
   // TODO(GG): use a standard approach
   if (!cond) {
      printf("\nTest failed: %s", msg);
      assert(cond && msg);
   }
...
struct SimpleReadHeader
{
   ...
   {
      size_t size = sizeof(SimpleReadHeader) +
         (sizeof(SimpleKey) * (numOfKeysToRead - 1));
      char *pBuf = new char[size];
      memset(pBuf, 0, size);
   ...
   {
      return sizeof(SimpleReadHeader) +
...
private:
                    ...
                    std::list<SimpleReplyHeader*>& outReplies)
   {
    ...
    // TODO(GG): can be used to support multi-executions of the test on the same
    // blockchain
   const int64_t m_testPrefix;
   ...
   void createAndInsertRandomConditionalWrite()
   {
      ...
      SimpleConditionalWriteHeader* pHeader =
         SimpleConditionalWriteHeader::alloc(numberOfKeysInReadSet,
                                             numberOfWrites);
      ...
      for (size_t i = 0; i < numberOfKeysInReadSet; i++) {
         ...
         memcpy(pReadKeysArray[i].key, &k, sizeof(size_t));
      }
      ...
      for (size_t i = 0; i < numberOfWrites; i++) {
         ...
         size_t v = rand();
         memcpy(pWritesKVArray[i].key, &k, sizeof(size_t));
         memcpy(pWritesKVArray[i].val, &v, sizeof(size_t));
      }
      ...
      for (BlockId i = readVer + 1;
           (i <= m_lastBlockId) && !foundConflict;
           i++) {
         SimpleBlock *currBlock = m_internalBlockchain[i];
         for (size_t a = 0;
              (a < numberOfKeysInReadSet) && !foundConflict;
              a++) {
            for (size_t b = 0;
                 (b < currBlock->numberOfItems) && !foundConflict;
                 b++) {
               if (memcmp(pReadKeysArray[a].key,
                          currBlock->items[b].key, KV_LEN) == 0) {
                  foundConflict = true;
               }
            }
         }
   ...
   void createAndInsertRandomRead()
   {
      ...
      for (size_t i = 0; i < numberOfReads; i++) {
         size_t k = rand() % NUMBER_OF_KEYS;
         memcpy(pHeader->keys[i].key, &k, sizeof(size_t));
      }
      ...
      for (size_t i = 0; i < numberOfReads; i++) {
         ...
         if (p != m_map.end() &&
             (pHeader->readVerion >= p->first.blockId) &&
...
public:
                               ...
                               size_t &outReplySize) const
   {
      ...
      for (size_t i = 0;
           !hasConflict && i < pCondWrite->numberOfKeysInReadSet;
           i++) {
      ...
      if (!hasConflict) {
         ...
         unordered_set<KeyValuePair> updates;
         ...
         blockAppender.addBlock(updates, newBlockId);
...
void run(IClient *client, const size_t numOfOperations)
{
   ...
   Internal::InternalTestsBuilder::createRandomTest(
      numOfOperations, 1111, INT64_MIN, requests, expectedReplies);
