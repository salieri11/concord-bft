...
import { ClrStepper, ClrWizardPage } from '@clr/angular';
...
const RegionCountValidator: ValidatorFn = (fg: FormGroup): ValidationErrors | null => {
  ...
  return nodes !== count ? { 'countIsCorrect': false } : null;
};
...
@Component({
  selector: 'concord-deploy-stepper',
  templateUrl: './deploy-stepper.component.html',
  styleUrls: ['./deploy-stepper.component.scss']
})
export class DeployStepperComponent implements AfterViewInit {
  @ViewChild('wizard', { static: false }) wizard: ClrStepper;
  ...
  ngAfterViewInit() {
    this.wizard['stepperService'].activeStep.source.subscribe(
      info => this.handlePageChange(info)
    );
  }
  ...
  addClientGroup(groupName: string = '') {
    ...
    setTimeout(() => {
      this.clientsForm.nativeElement.querySelector('#deploy-page-clients-group-' + g + '-name-input').focus();
    }, 100);
  }
  ...
  initForm(): FormGroup {
    ...
    return new FormGroup({
      ...
      nodeSizingTemplate: new FormGroup({
  ...
  addReplicaSize(template: NodeTemplateFormResponse) {
    ...
    this.selectedSizing = template.size;
  }
  ...
  private handlePageChange(currentStep: string) {
    switch (currentStep) {
      case 'details':
        setTimeout(() => {
      ...
      case 'clients':
        if (this.clientGroups.length === 0) {
          setTimeout(() => this.addClientGroup(), 10);
        }
        break;
    }
  }
