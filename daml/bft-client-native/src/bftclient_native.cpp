...
/*
 * Class:
 * com.digitalasset.daml.on.vmware.write.service.bft.BftConcordClientPoolJni
 * Method:    initialize
 * Signature: (Ljava/lang/String;)S
 */
JNIEXPORT jshort JNICALL
Java_com_digitalasset_daml_on_vmware_write_service_bft_BftConcordClientPoolJni_initialize(
...
Java_com_digitalasset_daml_on_vmware_write_service_bft_BftConcordClientPoolJni_sendRequest(
    JNIEnv *env, jobject obj, jbyteArray req, jlong timeout_millis,
    jboolean pre_exec, jstring correlationId) {
  ...
  int result = BFTClient_send_request(
      bftClient, reinterpret_cast<const char *>(reqBytes), reqLen, pre_exec,
      timeout_millis, correlationIdString);
 ...
 */
JNIEXPORT jint JNICALL
Java_com_digitalasset_daml_on_vmware_write_service_bft_BftConcordClientPoolJni_currentHealth(
...
Java_com_digitalasset_daml_on_vmware_write_service_bft_BftConcordClientPoolJni_close(
    JNIEnv *env, jobject obj) {
  uint16_t bftClient = get_native_handle(env, obj);
