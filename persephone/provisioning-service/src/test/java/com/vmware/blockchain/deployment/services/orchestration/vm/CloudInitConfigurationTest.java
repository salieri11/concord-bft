...
public class CloudInitConfigurationTest {
    ...
    // Test to verify with a Container Registry having port
    @Test
    void testContainerRegSelfSignedCertWithPort() throws IOException {
        ...
        when(containerRegistry.getAddress()).thenReturn("https://127.0.0.1:443");
        when(containerRegistry.getTransportSecurity()).thenReturn(transportSecurity);

        // mock cloudInitData details with above change to Container Registry
        CreateComputeResourceRequestV2.CloudInitData cloudInitData =
                mock(CreateComputeResourceRequestV2.CloudInitData.class);
        when(cloudInitData.getContainerRegistry()).thenReturn(containerRegistry);
        when(cloudInitData.getNotaryServer()).thenReturn(notaryServer);
        ...
        File file = new File(
                getClass().getClassLoader().getResource("userdata/user-data-with-container-reg-cert.txt")
                        .getFile());
        var expected = new String(Files.readAllBytes(file.toPath()));
