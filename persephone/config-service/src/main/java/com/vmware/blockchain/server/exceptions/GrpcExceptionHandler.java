    ...
    private class ExceptionHandlingServerCallListener<ReqT, RespT>
        ...
        private void handleException(Exception exception, ServerCall<ReqT, RespT> serverCall, Metadata metadata) {
            if (exception instanceof ConfigServiceException) {
                ConfigServiceException ex = (ConfigServiceException) exception;
                serverCall.close(ex.getErrorCode().getGrpcStatus().withDescription(ex.getDetails())
                                                                                       .withCause(exception), metadata);
            } else {
                serverCall.close(ErrorCode.UNKNOWN_ERROR.getGrpcStatus().withDescription(exception.getMessage()),
                                                                                                              metadata);
            }
        }
    }
