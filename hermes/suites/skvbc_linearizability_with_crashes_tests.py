...
#         SkvbcLinearizabiltyWithCrashesTests
#
"""Test SKVBC linearizability with crashes"""
...
@describe()
def test_skvbc_linearizability_with_crashes(fxProduct, bft_network):
    trio.run(_test_skvbc_linearizability_with_crashes, bft_network)


@with_timeout
async def _test_skvbc_linearizability_with_crashes(bft_network):
    skvbc_chaos_test = SkvbcChaosTest()
    log.info("Running SKVBC linearizability test with crashes...")
    await skvbc_chaos_test.test_wreak_havoc(
        bft_network=bft_network,
        already_in_trio=True
    )
    log.info("SKVBC linearizability test with crashes: OK.")