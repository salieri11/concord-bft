...
def test_publish_as_other(fxBlockchain):
    ...
    '''
    samples = time_helper.get_samples()
    ...
    log.info("Attempting to move {} to {} from {}".format(
        target, test_target_value, original_target_value))
    ...
    for n in range(1, len(samples)+1):
        ...
        output = time_helper.run_conc_time(concord, "-l --app /concord/config-local/application.config \
        ...
        new_samples = time_helper.extract_samples_from_response(output)
        if new_samples.get(target) == test_target_value:
            ...
            assert new_samples.get(
                target) < test_target_value, "Time moved beyond our test bounds"
...
def find_source_container(source):
    ...
    while container:
        output = time_helper.run_conc_time(container, "--app /concord/config-local/application.config \
                                         --depl /concord/config-local/deployment.config \
                                         --secr /concord/config-local/secrets.config \
                                         -t 0 -n")
        if output.find(source) > 0:
            log.info("Found time source '{}' in container '{}'".format(
                source, container))
            return container
        else:
            containerIndex += 1
...
def run_faulty_clock(faulty_time):
    ...
    # necessary.
    slow_sources, healthy_sources, fast_sources = time_helper.find_faulty_clocks(
        expectedUpdatePeriodSec)
    allowed_faults = time_helper.max_faulty_clocks()
    ...
    if len(slow_sources) + len(fast_sources) < allowed_faults:
        ...
        test_target_source = random.choice(
            slow_sources + healthy_sources + fast_sources)
    else:
    ...
    summary, samples = time_helper.get_summary_and_samples()
    ...
    for attempt in range(1, attempts+1):
        ...
        expectUpdate = test_target_time > samples.get(
            test_target_source) + expectedUpdatePeriodSec
        ...
        output = time_helper.run_conc_time(concordContainer, args="-g -l --app /concord/config-local/application.config \
                                                           --depl /concord/config-local/deployment.config \
                                                           --secr /concord/config-local/secrets.config \
                                                     --source {} -t {}".format(test_target_source, test_target_time))
        summary = time_helper.extract_time_summary_response(output)
        samples = time_helper.extract_samples_from_response(output)
