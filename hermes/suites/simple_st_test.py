...
SstSetupFixture = collections.namedtuple(
    "SstSetupFixture", "to, funcPref, gas, existing_transactions")
...
def send_data(hermesData, sstParams, blockchainId, count, contractAddress, maxRetries):
    ...
    while i < count:
        try:
            ...
            txres = request.rpc.sendTransaction(
                sstParams.to, sstParams.funcPref + rand, sstParams.gas, contractAddress)
            if retries == 0:
                i += 1
            retries = 0
        except:
            log.debug("send_data fail, retries:{0}, maxRetries:{1}".format(
                retries, maxRetries))
            retries += 1
...
def check_data(path, blockToStart, blocksCount):
    try:
        ...
        for replicaId in range(1, 5):
            pParam = "-p={0}:{1}".format(blockToStart,
                                         blockToStart + blocksCount)
            cmd = ' '.join([toolPath, pathParam, opParam, pParam])
            container = util.blockchain.eth.get_concord_container_name(
                replicaId)
            ...
            if length == 0:
                ...
                return False
            log.info("Data length from replica {0} is {1} bytes".format(
                replicaId, length))
...
def send_async(hermesSettings, sstParams, blockchainId, transactions, contractAddress, maxRetries, threadCount=10):
    ...
    for i in range(0, threadCount):
        t = Thread(target=send_data, args=(hermesSettings, sstParams,
                                           blockchainId, txPerThread, contractAddress, maxRetries))
        t.start()
        threads.append(t)
...
def test_kill_replica(request, fxSstSetup, fxHermesRunSettings, fxProduct, fxBlockchain, fxConnection):
    try:
        ...
        req = createRequest(
            hermesSettings, fxBlockchain.blockchainId, request.node.name)
        ...
        send_async(hermesSettings, fxSstSetup,
                   fxBlockchain.blockchainId, transactions, contractAddress, 1)

        # Is this a alternate way of getting instance of aleady running product
        # product = Product(hermesData["hermesCmdlineArgs"],
        #                    hermesData["hermesUserConfig"])

        assert fxProduct.product.kill_concord_replica(
            2), "Failed to kill replica 2"

        path = fxProduct.product.cleanConcordDb(2)
        assert path, "Failed to clean RocksDb"
        assert fxProduct.product.start_concord_replica(
            2), "Failed to start replica 2"
        assert sleep_and_check(120, 30, 360, transactions +
                               fxSstSetup.existing_transactions), "Data check failed"
...
def test_pause_replica(request, fxSstSetup, fxHermesRunSettings, fxBlockchain, fxProduct, fxConnection):
    try:
        ...
        assert fxProduct.product.kill_concord_replica(
            2), "Failed to kill replica 2"
        assert fxProduct.product.kill_concord_replica(
            3), "Failed to kill replica 3"
        assert fxProduct.product.kill_concord_replica(
            4), "Failed to kill replica 4"
        ...
        assert fxProduct.product.start_concord_replica(
            2), "Failed to start replica 2"
        assert fxProduct.product.start_concord_replica(
            3), "Failed to start replica 3"
        assert fxProduct.product.start_concord_replica(
            4), "Failed to start replica 4"
        ...
        transactions_1 = 1000
        send_async(hermesSettings, fxSstSetup, fxBlockchain.blockchainId,
                   transactions_1, contractAddress, 1)

        assert fxProduct.product.pause_concord_replica(
            3), "Failed to suspend replica"

        transactions_2 = 400
        # this batch will go slower since only 3 replicas are up
        send_async(hermesSettings, fxSstSetup, fxBlockchain.blockchainId,
                   transactions_2, contractAddress, 3)

        assert fxProduct.product.resume_concord_replica(
            3), "Failed to resume replica"
        assert sleep_and_check(90, 10, 200, transactions_1 + transactions_2 +
                               fxSstSetup.existing_transactions), "Data check failed"

        # check all blocks
        assert check_data(path, 0, transactions_1 + transactions_2 +
                          fxSstSetup.existing_transactions), "Data check failed"
