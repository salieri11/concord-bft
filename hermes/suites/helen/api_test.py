...
@pytest.mark.smoke
def test_getABlockchain_valid(restRequest):
   '''
   Test GET /blockchains/{bid}, which gets details for a given blockchain.
   '''
   blockchainId = restRequest.getBlockchains()[0]["id"]
   details = restRequest.getBlockchainDetails(blockchainId)
   checkBlockchainFields(restRequest, details)


@pytest.mark.smoke
def test_getABlockhain_invalid_uuid(restRequest):
   '''
...
@pytest.mark.smoke
def test_getABlockhain_invalid_uuid_format(restRequest):
   '''
