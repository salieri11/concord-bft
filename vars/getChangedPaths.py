...
def main():
  try:
    print("Detecting file changes in this MR...")
    ...
    diffOutput = subprocess.run([
        'git', 'diff', 'origin/master', '--name-only', '--exit-code'
    ...
    for componentName in componentsMap:
      componentsMap[componentName] = False
    ...
    for path in paths:
      ...
      if folderName is not None:
        if folderName in componentsMap:
          ...
          componentsMap[folderName] = True
          print(f"{folderName} changed ({path})")
        else:
          componentsMap["uncaught"] = True # unknown directory change => build/test all
          print(f"{folderName} changed ({path}) => UNCAUGHT directory change")
      for subComponent in subComponents:
        if path.startswith(subComponent):
          componentsMap[subComponent] = True
    ...
    if len(paths) == 0: # no functional changes in this MR (e.g. the changes are only *.md, README, etc.)
      componentsMap["nothing"] = True # No need to bulid or test; just pass the pipeline right away
    ...
    if componentsMap["uncaught"] is True:
      print("Uncaught path change is detected; this run will build and test EVERYTHING.")
      for componentName in componentsMap: componentsMap[componentName] = True
      componentsMap["nothing"] = False
