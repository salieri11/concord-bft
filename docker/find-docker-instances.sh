...
# file currently requires an IP to be specified at start time. We have to wait
# for Docker to assign those IPs first. Thus, this script waits for IPs of the
# concord nodes, and then insert them into the Concord config file.
#
# Arguments:
#  1: The path to the Concord configuration for this node.
