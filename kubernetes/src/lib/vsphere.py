...
class Vsphere:
    ...
    def check_if_subfolder_exits(self, folder_name):
        """
        Check if any subfolder's exits within given folder

        :param str folder_name: Name of folder
        :rtype True if subfolder exists else False
        """
        folder = self.get_folder(folder_name)
        for entity in folder.childEntity:
            if isinstance(entity, vim.Folder) == True:
                return True
        return False

    def get_vms_from_folder(self, folder_name):
        """
        Return all vms from a given folder

        :param str folder_name: Name of the folder
        :rtype list of all the vm's objects
        """
        folder = self.get_folder(folder_name)
        vms = []
        for entity in folder.childEntity:
            if isinstance(entity, vim.VirtualMachine) == True:
                vms.append(entity)
        return vms

    def get_vm_task_collector(self, vm):
        """
        Get task iterator for a vm

        :param vim.VirtualMachine vm: vm Vim object
        rtype task collector iterator
        """
        try:
            filterspec = vim.TaskFilterSpec(entity=
                    vim.TaskFilterSpec.ByEntity(entity=vm, recursion="self"))
            collector = self.content.taskManager.CreateCollectorForTasks(filterspec)
            return collector
        except Exception as e:
            self._log.exception("Error connecting to vSphere: %s", str(e))
            return None
