...
public final class AthenaTcpConnection implements IAthenaConnection {
    ...
    public AthenaTcpConnection(AthenaProperties config, String host, int port) throws IOException {
        ...
        try {
            ...
            logger.error("Error creating TCP connection with Athena. Host= " + host + ", port= " + port);
            throw new UnknownHostException();
        } catch (IOException e) {
            logger.error("Error creating input/output stream with Athena. Host= " + host + ", port= " + port);
    ...
    /**
     * Reads responses from Athena. Athena sends the size of the response before the actual response
     */
    @Override
    public byte[] receive() {
        try {
